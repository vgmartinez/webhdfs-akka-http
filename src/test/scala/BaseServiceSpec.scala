...
trait BaseServiceSpec extends WordSpec with Matchers with ScalatestRouteTest with Routes with MigrationConfig with Base{
  protected val log: LoggingAdapter = NoLogging

  import driver.api._

  val testUsers = Seq(
    UserEntity(Some(1), "userName1", "password1", 21, 1),
    UserEntity(Some(2), "userName2", "password2", 22, 2),
    UserEntity(Some(3), "userName3", "password3", 24, 1)
  )
